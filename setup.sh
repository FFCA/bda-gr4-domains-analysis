...
basename="$(dirname "$0")/"
sparkdir=$basename"services/pyspark-analyse/"
...
# Case: File stated as param is valid csv => copy file to the repository's root dir making sure it is named correctly
  ...
  echo "Copying file" $1 "to" $sparkdir$csvFile "(updates existing file)"
  cp $1 $sparkdir$csvFile
# Case: No or no existing file stated as param but csv could be found at the required location
elif [ -e $sparkdir$csvFile ]; then
  echo "Existing" $sparkdir$csvFile "will be used!"
# No data at the repository's root and no existing file stated as param  => exit with information
else
  echo "Failed to find" $sparkdir$csvFile"!"
  ...
  echo "  2. Download the data and store it manually as:" $sparkdir$csvFile "(relative path to the directory this script has been run from)"
